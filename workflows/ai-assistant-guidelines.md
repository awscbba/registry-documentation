...
#### Deployment Process:
```bash
# ⚠️ NEVER deploy from local machine!
# Deployments are handled through CI/CD pipeline (GitHub Actions/CodeCatalyst)
# Code changes in registry-api/ trigger automatic container rebuild and deployment
# Push to feature branch → Create PR → Merge to main → Automatic deployment
```
...
5. **CI/CD pipeline** automatically rebuilds containers and deploys to Lambda

##### Deployment Dependencies:
- **Source Code**: `registry-api/` repository
- **Infrastructure**: `registry-infrastructure/` repository  
- **CI/CD Pipeline**: GitHub Actions or CodeCatalyst handles deployments
- **Container Registry**: ECR automatically managed by pipeline

##### Common Issues:
- **Code changes not reflected**: Wait for CI/CD pipeline to complete deployment
